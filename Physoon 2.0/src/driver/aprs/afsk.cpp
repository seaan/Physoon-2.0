...
static const uint16_t PHASE_DELTA_2200 = (((TABLE_SIZE * 2200UL) << 7) / PLAYBACK_RATE); //*******2200UL????
...
inline static bool afsk_is_fifo_full(void)
{
  return (((sample_fifo_head + 1) % SAMPLE_FIFO_SIZE) == sample_fifo_tail);
}

inline static bool afsk_is_fifo_full_safe(void)
{
...
inline static bool afsk_is_fifo_empty(void)
{
  return (sample_fifo_head == sample_fifo_tail);
}

inline static bool afsk_is_fifo_empty_safe(void)
{
  Disable_global_interrupt();
...
inline static uint8_t afsk_fifo_out(void)
{
...
inline static uint8_t afsk_fifo_out_safe(void)
{
  Disable_global_interrupt();
